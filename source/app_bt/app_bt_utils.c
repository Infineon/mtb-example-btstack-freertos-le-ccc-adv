...
/**
 * Function Name: get_bt_scan_mode_name
 *
 * Function Description:
 *   @brief The function converts the wiced_bt_ble_scan_type_t enum value
 *          to its corresponding string literal. This will help the programmer
 *          to debug easily with log traces without navigating through the
 *          source code.
 *
 *   @param wiced_bt_ble_scan_type_t type: Bluetooth scan type
 *
 *   @return wiced_bt_ble_scan_type_t
 *
 */
const char *get_bt_scan_type_name(wiced_bt_ble_scan_type_t type)
{

    switch ( (int)type )
    {

    CASE_RETURN_STR(BTM_BLE_SCAN_TYPE_NONE)
    CASE_RETURN_STR(BTM_BLE_SCAN_TYPE_HIGH_DUTY)
    CASE_RETURN_STR(BTM_BLE_SCAN_TYPE_LOW_DUTY)
    CASE_RETURN_STR(BTM_BLE_SCAN_TYPE_LIMITED_HIGH_DUTY)
    CASE_RETURN_STR(BTM_BLE_SCAN_TYPE_LIMITED_LOW_DUTY)
    }
